...
// resetTestState is used to reset any global state before tests
func resetTestState() {
	// Add any state reset needed for tests
}
...
func TestDecode(t *testing.T) {
	...
	t.Run("nil bitcom", func(t *testing.T) {
		...
		// Test nil Bitcom
		var nilBitcom *bitcom.Bitcom
		result := Decode(nilBitcom)
	...
	t.Run("empty protocols", func(t *testing.T) {
		...
		emptyBitcom := &bitcom.Bitcom{
			Protocols: []*bitcom.BitcomProtocol{},
		}
		result := Decode(emptyBitcom)
		require.NotNil(t, result, "Result should be an empty slice, not nil")
...
func TestDecodeWithSigmaTestVector(t *testing.T) {
	...
	var bitcomData *bitcom.Bitcom
	for i, output := range tx.Outputs {
		// Use bitcom to decode each output's script
		decodedBitcom := bitcom.Decode(output.LockingScript)
